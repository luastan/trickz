<template>
  <div class="w-full relative">
    <pre class="bg-gray-200 fancy-scrollbar text-black w-full relative"><tricks-copy-code/><code :class="classes" v-html="code"></code></pre>
    <smart-input
      v-for="(defaultValue, variable) in smartVariables"
      :key="variable"
      :variable="variable"
...
import '~/assets/css/code-snippets-theme.css';
...
export default {
  ...
  computed: {
    code() {
      return this.decodedCode.replaceAll(
        findTplRegex,
  ...
  mounted() {
    if (!this.$isServer) {
      ...
      const newVariables = {}
      const regexResults = this.decodedCode.matchAll(findTplRegex);
      for (const match of regexResults) {
        const variable = match[1];
        console.log(variable)
        if (!variable.includes('__')) {
          this.smartVariables[variable] = match[2].trim();
        }
      }
      // this.smartVariables = newVariables;
    }
